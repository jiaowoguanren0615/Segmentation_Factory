...
class CrossFormerPP(nn.Module):
    ...
    def compute_group_size(self, group_size=[7, 7, 7, 7], depths=[2, 2, 6, 2], resolution=[56, 56],
                           group_type='constant'):
...
def crossformerpp_small(**kwargs):
    embed_dim = 64
    depths = [2, 2, 18, 2]
    num_heads = [2, 4, 8, 16]
    group_size = [4, 4, 14, 7]
    crs_interval = [4, 4, 1, 1]
    model = CrossFormerPP(embed_dim=embed_dim, depths=depths, num_heads=num_heads, group_size=group_size,
                        crs_interval=crs_interval, **kwargs)
    return model

def crossformerpp_base(**kwargs):
    embed_dim = 96
    depths = [2, 2, 18, 2]
    num_heads = [3, 6, 12, 24]
    group_size = [4, 4, 14, 7]
...
def crossformerpp_large(**kwargs):
    embed_dim = 128
    depths = [2, 2, 18, 2]
    num_heads = [4, 8, 16, 32]
    group_size = [4, 4, 14, 7]
