...
def crossformer_tiny(**kwargs):
    embed_dim = 64
    depths = [1, 1, 8, 6]
    num_heads = [2, 4, 8, 16]
    group_size = [7, 7, 7, 7]
    crs_interval = [8, 4, 2, 1]
    model = CrossFormer(embed_dim=embed_dim, depths=depths, num_heads=num_heads, group_size=group_size,
                        crs_interval=crs_interval, **kwargs)
    return model


def crossformer_small(**kwargs):
    embed_dim = 96
    depths = [2, 2, 6, 2]
    num_heads = [3, 6, 12, 24]
    group_size = [7, 7, 7, 7]
