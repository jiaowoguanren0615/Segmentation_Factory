...
from datasets.extra_transform import ExtCompose, ExtNormalize, RandomResizedCrop, ExtToTensor
...
def visualize_dataset_sample(dataset, root, split='val', batch_size=4):
    ...
    pics = make_grid(images, nrow=4).to(torch.uint8).numpy().transpose((1, 2, 0))
    fig = go.Figure(data=go.Image(z=pics))
    fig.show()
    # plt.imshow(pics)
    # plt.show()
